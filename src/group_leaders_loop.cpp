...
void mango::problem::group_leaders_loop() {
  ...
  std::cout << "Proc " << mpi_partition.get_rank_world() << " in entering group_leaders_loop.\n";
  ...
  while (keep_going) {
    ...
    if (data < 0) {
      std::cout << "proc " << mpi_partition.get_rank_world() << " (a group leader) is exiting.\n";
      keep_going = false;
    } else {
      std::cout << "proc " << mpi_partition.get_rank_world() << " (a group leader) is starting finite-difference gradient calculation.\n";
