subroutine mango_finite_difference_gradient(problem, objective_function, state_vector, base_case_objective_function, gradient)
  ...
  double precision :: f
  double precision, allocatable :: perturbed_state_vector(:), objective_functions(:), state_vectors(:,:)
  integer :: ierr, data(1)
  logical :: failed

  !-----------------------------------------------------------------

  print "(a,i3)","Hello from mango_finite_difference_gradient from proc",problem%mpi_rank_world
  ...
  do j_evaluation = 1, N_evaluations
     if (mod(j_evaluation-1, problem%N_worker_groups) == problem%mpi_rank_group_leaders) then
        call objective_function(problem, state_vectors(:,j_evaluation), f, failed)
  ...
  if (problem%proc0_world) then
     call mpi_reduce(MPI_IN_PLACE, objective_functions, N_evaluations, MPI_DOUBLE_PRECISION, MPI_SUM, 0, problem%mpi_comm_group_leaders, ierr)
  else
     call mpi_reduce(objective_functions, objective_functions, N_evaluations, MPI_DOUBLE_PRECISION, MPI_SUM, 0, problem%mpi_comm_group_leaders, ierr)
  end if
  ...
  if (problem%proc0_world) then
     do j_evaluation = 1, N_evaluations
        ...
        write (problem%output_unit,"(a,es24.15)") ",", objective_functions(j_evaluation)
        call flush(problem%output_unit)
     end do
  end if

  ! Evaluate the finite differences
  base_case_objective_function = objective_functions(1)
  if (problem%centered_differences) then
     do j_parameter = 1, problem%N_parameters
        gradient(j_parameter) = (objective_functions(j_parameter+1) - objective_functions(j_parameter+1+problem%N_parameters)) &
             / (2 * problem%finite_difference_step_size)
     end do
  else
     do j_parameter = 1, problem%N_parameters
        gradient(j_parameter) = (objective_functions(j_parameter+1) - base_case_objective_function) / problem%finite_difference_step_size
     end do
  end if

  ! Clean up
  deallocate(perturbed_state_vector, state_vectors, objective_functions)

end subroutine mango_finite_difference_gradient
